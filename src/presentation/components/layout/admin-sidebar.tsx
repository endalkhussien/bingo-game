...
import {
  ...
  Bell, LogOut, Ticket, KeyRound, Shield,
} from 'lucide-react';
...
import { getRoleLabel, isVendorRole } from '@/shared/roles';
import { VENDOR_HOME } from '@/shared/admin-routes';

/** Shop admin — runs the hall after TOL activation. */
...
/** Vendor — TOL for shop admin + activity board only. */
const vendorNav: Array<{ href: string; label: string; icon: typeof Shield; badge?: boolean }> = [
  { href: VENDOR_HOME, label: 'Vendor Board', icon: Shield },
];
...
export function AdminSidebar() {
  ...
  const isVendor = user && isVendorRole(user.role);

  const navItems = isVendor ? vendorNav : shopAdminNav;
