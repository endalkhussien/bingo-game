...
export const mockHandlers: Record<string, (...args: unknown[]) => unknown> = {
  ...
  'audio:play-paths': async (paths: unknown) => {
    const { playRelativePathsDirect } = await import('@/presentation/lib/amharic-audio');
    const list = Array.isArray(paths) ? paths.map(String) : [];
    if (!list.length) return { success: false, error: 'mock: no paths' };
    const ok = await playRelativePathsDirect(list);
