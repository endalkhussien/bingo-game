...
import { seedDatabase, ensureVendorUser, ensureShopAdminUser, removeSeededDemoAgent } from '../../src/infrastructure/database/seed';
...
export async function initDatabase(): Promise<BetterSQLite3Database<typeof schema>> {
  ...
  try {
    ...
    await removeSeededDemoAgent(database);
    return database;
  } catch (err) {
